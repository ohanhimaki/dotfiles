...
public class InstallService
{
    ...
    public bool WingetInstall(string package, string args = "")
    {
        ...
        return RunCommand("winget", $"install --id {package} -e --silent --accept-source-agreements --accept-package-agreements {args}");
    }
    ...
    private bool RunCommand(string command, string args)
    {
        ...
        try
        {
            ...
            if (!success)
            {
                Log($"  ✗ Command failed with exit code {proc?.ExitCode}", LogLevel.Warning);
            }
